...
class ThreadInfo {
    ...
    private ThreadInfo(Environment pEnv, messageOutput pmsgout, ThreadReference thread) {
        this.thread = thread;
        this.Env = pEnv;
    ...
    private static void initThreads(Environment pEnv, messageOutput pmsgout) {
        if (!gotInitialThreads) {
            Iterator iter = pEnv.vm().allThreads().iterator();
            while (iter.hasNext()) {
                ThreadReference thread = (ThreadReference)iter.next();
                threads.add(new ThreadInfo(pEnv, pmsgout, thread));
            }
            gotInitialThreads = true;
        }
    }

    static void addThread(Environment pEnv, messageOutput pmsgout, ThreadReference thread) {
        synchronized (threads) {
            initThreads(pEnv, pmsgout);
            ThreadInfo ti = new ThreadInfo(pEnv, pmsgout, thread);
            ...
            if (getThreadInfo(pEnv, pmsgout, thread) == null) {
                threads.add(ti);
            }
        }
    }

    static void removeThread(Environment pEnv, messageOutput pmsgout, ThreadReference thread) {
        if (thread.equals(ThreadInfo.current)) {
            ...
            setCurrentThread(pEnv, pmsgout, null);

            //MessageOutput.println();
            //MessageOutput.println("Current thread died. Execution continuing...",
            //                      currentThreadName);
        }
        threads.remove(getThreadInfo(pEnv, pmsgout, thread));
    }

    static List threads(Environment pEnv, messageOutput pmsgout) {
        synchronized(threads) {
            initThreads(pEnv, pmsgout);
    ...
    static void invalidateAll(Environment pEnv, messageOutput pmsgout) {
        current = null;
        group = null;
        synchronized (threads) {
            Iterator iter = threads(pEnv, pmsgout).iterator();
            while (iter.hasNext()) {
    ...
    static void setCurrentThread(Environment pEnv, messageOutput pmsgout, ThreadReference tr) {
        if (tr == null) {
            setCurrentThreadInfo(null);
        } else {
            ThreadInfo tinfo = getThreadInfo(pEnv, pmsgout, tr);
    ...
    static ThreadGroupReference group(Environment pEnv) {
	if (group == null) {
            // Current thread group defaults to the first top level
            // thread group.
	    setThreadGroup((ThreadGroupReference)
                           pEnv.vm().topLevelThreadGroups().get(0));
	}
        return group;
    }
    
    static ThreadInfo getThreadInfo(Environment pEnv, messageOutput pmsgout,  long id) {
    ...
    static ThreadInfo getThreadInfo(Environment pEnv, messageOutput pmsgout, ThreadReference tr) {
        return getThreadInfo(pEnv,pmsgout, tr.uniqueID());
    }

    static ThreadInfo getThreadInfo(Environment pEnv, messageOutput pmsgout, String idToken) {
        ...
        try {
            ...
            tinfo = getThreadInfo(pEnv, pmsgout, threadId);
